  ...
  const fetchOrders = async () => {
    try {
      ...
      if (stored) {
        ...
        setOrders(allOrders.filter(order => order.status === 'processing'));
      } else {
        setOrders([]);
      }
    } catch (error) {
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (stored) {
          ...
          setOrders(updatedOrders.filter(o => o.status === 'processing'));
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Orders currently being prepared for shipment</p>
        </div>
      </div>
                ...
                <tr>
                  <td colSpan="7" className="text-center py-12">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-sm text-gray-400">Orders in processing will appear here</p>
                    </div>
                  </td>
                </tr>
              ) : (
